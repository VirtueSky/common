...
namespace Lean.Touch
{
	...
	public class LeanMultiSwipe : MonoBehaviour
	{
		...
		public float ScaledDistanceThreshold { set { scaledDistanceThreshold = value; } get { return scaledDistanceThreshold; } } [SerializeField] private float scaledDistanceThreshold = 50.0f;
		...
		public float ParallelAngleThreshold { set { parallelAngleThreshold = value; } get { return parallelAngleThreshold; } } [SerializeField] private float parallelAngleThreshold = 20.0f;

		/// <summary>This allows you to set the minimum pinch distance for the OnSwipeIn and OnSwipeOut events to be fired.</summary>
		public float PinchScaledDistanceThreshold { set { pinchScaledDistanceThreshold = value; } get { return pinchScaledDistanceThreshold; } } [SerializeField] private float pinchScaledDistanceThreshold = 100.0f;

		// Called when a multi-swipe occurs
		public FingerListEvent OnFingers { get { if (onFingers == null) onFingers = new FingerListEvent(); return onFingers; } } [SerializeField] private FingerListEvent onFingers;

		// Called when a multi-swipe occurs where each finger moves parallel to each other (Vector2 = ScaledDirection)
		public Vector2Event OnSwipeParallel { get { if (onSwipeParallel == null) onSwipeParallel = new Vector2Event(); return onSwipeParallel; } } [SerializeField] private Vector2Event onSwipeParallel;

		// Called when a multi-swipe occurs where each finger pinches in (Float = ScaledDistance)
		public FloatEvent OnSwipeIn { get { if (onSwipeIn == null) onSwipeIn = new FloatEvent(); return onSwipeIn; } } [SerializeField] private FloatEvent onSwipeIn;

		// Called when a multi-swipe occurs where each finger pinches out (Float = ScaledDistance)
		public FloatEvent OnSwipeOut { get { if (onSwipeOut == null) onSwipeOut = new FloatEvent(); return onSwipeOut; } } [SerializeField] private FloatEvent onSwipeOut;
...
namespace Lean.Touch.Editor
{
	...
	public class LeanMultiSwipe_Editor : CwEditor
	{
